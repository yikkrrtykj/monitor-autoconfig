...
| 告警及巡检群名称 | 公司和各比赛现场填写同一个飞书群名称；每套监控使用各自的“赛事名称”标识回复和告警 |

飞书企业自建应用审批通过后，在 `/control` 的“告警”区填写 App ID、App Secret，
把应用机器人加入告警群，再点“应用配置”。旧版只写在 `.env` 的凭据会自动带入
后台输入框。公司和每个比赛现场可使用同一 App ID、App Secret 和群名称，但各自填写
不同的“赛事名称”。每套监控主动读取同一群的 `@机器人` 命令，只查询自己的本地数据，
并在回复标题前显示赛事名称；因此一次命令可收到公司及所有在线比赛现场的结果。
支持 `查设备 <名称/IP>`、`查光功率 <名称/IP> [接口]`、
`查异常光功率 <名称/IP> [接口]`、`光功率巡检`、`上联冗余巡检` 和 `帮助`。
