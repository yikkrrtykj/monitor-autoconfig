...
BIGSCREEN_ISP_MAX_BANDWIDTH=ISP1:500,ISP2:500,ISP3:300  # ISP 流量图固定上限，单位 Mbps；也可只写 1000
...
4. 排错 snmpwalk：
   ...
   docker exec player-targets snmpwalk -v2c -c global <STAGE_IP> 1.3.6.1.2.1.17.4.3.1.2 | head
   docker exec player-targets snmpwalk -v2c -c global <STAGE_IP> 1.3.6.1.2.1.17.7.1.2.2.1.2 | head
   # 网关 ARP 是否含选手 IP
   docker exec player-targets snmpwalk -v2c -c global <GATEWAY_IP> 1.3.6.1.2.1.4.22.1.2 | head
   ```
...
**改了 .env 后某些数据不更新**
- 改 `TOURNAMENT_SWITCHES` / `PLAYER_GATEWAYS` / `PLAYER_SUBNETS` / `WIRELESS_SUBNETS` / `PLAYER_STATIC_TARGETS` / `PLAYER_WIRELESS_SCAN` → `player-targets` 每 60 秒自动读取 `.env`
