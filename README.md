...
`get.docker.com` 默认从 `download.docker.com` 拉包，国内服务器常超时，导致
`Unable to locate package docker-compose-plugin` / `group 'docker' does not exist`。
加 `--mirror Aliyun` 走阿里云镜像即可（脚本本身已含 `docker-compose-plugin`，不必再单独装）：

```bash
curl -fsSL https://get.docker.com -o get-docker.sh
sudo sh get-docker.sh --mirror Aliyun     # 海外服务器可去掉 --mirror Aliyun
...
# 国内用阿里云镜像；海外换成 https://download.docker.com/linux/centos/docker-ce.repo
