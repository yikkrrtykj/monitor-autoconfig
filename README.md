...
如果写 `1000`，告警线就是 `900 Mbps`。`apply-env.sh` 会把这个速率同步到 LibreNMS 的防火墙 WAN 口，让 LibreNMS 里的 `port_usage_perc` 按运营商带宽算。
...
LibreNMS poller 有周期，规则不是毫秒级触发；但规则页必须显示 Feishu transport，否则只会在 LibreNMS 里记录，不会推飞书。
