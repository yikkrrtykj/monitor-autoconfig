...
DIST_SWITCH_PING=SW:192.168.10.11-22   # 可写范围，自动显示为 SW1、SW2...
...
1. 规则页 `Transports` 不能是 `none`；改完 `.env` 后跑 `./apply-env.sh`，脚本会把 Feishu transport 自动绑到三条规则上。
2. 端口百分比必须按运营商带宽算。比如 300M 专线就填 `BIGSCREEN_ISP_MAX_BANDWIDTH=300`，再跑 `./apply-env.sh`，脚本会覆盖防火墙 WAN 口速率。
3. WAN 口要能被匹配到。优先在防火墙端口描述里写 `telecom/unicom/WAN`；如果端口没描述，就把真实端口名加入 `FIREWALL_WAN_IF_FILTER`，例如：
   ```bash
   FIREWALL_WAN_IF_FILTER=telecom,telcom,unicom,isp,WAN,xethernet0/24,ethernet0/2
   ```
...
- 通用做法：改完 `.env` 后跑 `./apply-env.sh`。它不拉镜像，只重建会读取 `.env` 的容器，并重跑 LibreNMS 规则/告警配置。
