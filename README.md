...
syslog 飞书推送默认由 `SYSLOG_ALERT_TYPES=native_vlan_mismatch,errdisable,loopback,dhcp_snooping` 控制。MAC 漂移、BPDU Guard 单独日志、storm-control 单独日志默认只进 Loki/LibreNMS，不推飞书；BPDU Guard 导致接口 err-disable 时，会合并显示在“接口被保护关闭”卡片里。
