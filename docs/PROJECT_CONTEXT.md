...
Feishu 隔离最小修复已实施：空/空白 EVENT_NAME 的群轮询和长连接群消息统一静默拒绝，明确 p2p 的长连接回退保留既有兼容；测试与文档已同步，待独立审计与生产验收，见 [隔离迭代记录](iterations/feishu-event-name-isolation.md)。不修改冻结 ISP、已收口批次、P3 刷新或设备删除逻辑。
