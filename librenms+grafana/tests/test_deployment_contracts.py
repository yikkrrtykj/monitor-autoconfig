...
def _render_prometheus_config(tmp_path: Path, auto_value: str) -> str:
    ...
    if not shell:
        windows_shell = Path(r"C:\Program Files\Git\usr\bin\sh.exe")
        shell = str(windows_shell) if windows_shell.is_file() else ""
    if not shell:
        raise RuntimeError("POSIX shell is unavailable")
    source = read("prometheus-gen-config.sh").split("exec /bin/prometheus", 1)[0]
    harness = tmp_path / "prometheus-gen-config.sh"
    ...
    env.update({
        ...
        "BIGSCREEN_ISP_AUTO_DISCOVER": auto_value,
...
def test_prometheus_auto_isp_uses_only_discovered_file_targets(tmp_path):
    for enabled in ("true", "1", "yes", "on", "TRUE", "YES", "ON"):
...
def test_prometheus_manual_isp_retains_public_ip_fallback(tmp_path):
    for index, disabled in enumerate(("false", "0", "no", "off", "")):
        rendered = _render_prometheus_config(tmp_path / f"manual-{index}", disabled)
...
def test_isp_auto_mode_and_native_inventory_are_wired_to_every_runtime_consumer():
    ...
    bridge = compose.split("  alertmanager-feishu-bridge:", 1)[1].split(
        "  snmp-exporter:", 1
    )[0]
...
def test_bigscreen_config_model_is_loaded_before_app_and_owns_only_pure_helpers():
    ...
    assert index.index("config/config-model.js?v=20260827a") < index.index("config/config-editor.js?v=20260911a")
...
def test_bigscreen_config_editor_is_loaded_before_app_and_owns_editor_state_and_dom():
    ...
    assert "config/config-editor.js?v=20260911a" in index
    assert index.index("config/config-model.js?v=20260827a") < index.index("config/config-editor.js?v=20260911a")
    assert index.index("config/config-editor.js?v=20260911a") < index.index("app.js?v=20260911a")
...
def test_bigscreen_dhcp_model_is_loaded_before_app_and_owns_only_pure_helpers():
    ...
    assert index.index("dhcp/dhcp-model.js?v=20260827a") < index.index("dhcp/dhcp-panel.js?v=20260911a")
...
def test_bigscreen_dhcp_panel_owns_page_state_dom_api_and_refresh_lifecycle():
    ...
    assert "dhcp/dhcp-panel.js?v=20260911a" in index
    assert index.index("dhcp/dhcp-model.js?v=20260827a") < index.index("dhcp/dhcp-panel.js?v=20260911a")
    assert index.index("dhcp/dhcp-panel.js?v=20260911a") < index.index("app.js?v=20260911a")
...
def test_all_bigscreen_pages_have_mobile_layout_contracts():
    ...
    assert "app.js?v=20260911a" in html
...
def test_loss_heatmap_splits_large_device_lists_into_two_columns():
    ...
    assert index.index("charts/loss-heatmap.js?v=20260826a") < index.index("app.js?v=20260911a")
...
def test_isp_and_evidence_use_business_specific_line_chart_facades():
    ...
    assert "ispChartMaxBps(result.name, !result.metadataConflict)" in isp_chart
    ...
    assert "evidence/evidence-panel.js?v=20260911a" in index
    ...
    assert index.index("charts/isp-chart.js?v=20260905a") < index.index("app.js?v=20260911a")
    assert index.index("charts/evidence-chart.js?v=20260826a") < index.index("evidence/evidence-panel.js?v=20260911a")
    assert index.index("evidence/evidence-panel.js?v=20260911a") < index.index("app.js?v=20260911a")
...
def test_incident_panel_owns_form_queries_rendering_and_request_lifecycle():
    ...
    assert "incident/incident-panel.js?v=20260905a" in index
    assert index.index("incident.js?v=20260904a") < index.index("incident/incident-panel.js?v=20260905a")
    assert index.index("incident/incident-panel.js?v=20260905a") < index.index("app.js?v=20260911a")
...
def test_wireless_panel_is_loaded_after_players_and_owns_page_controller():
    ...
    assert index.index("wireless/wireless-panel.js?v=20260827a") < index.index("app.js?v=20260911a")
...
def test_tournament_panel_is_loaded_after_players_and_owns_page_controller():
    ...
    assert index.index("tournament/tournament-panel.js?v=20260828a") < index.index("app.js?v=20260911a")
...
def test_infra_controller_owns_refresh_rendering_and_isp_lifecycle():
    ...
    assert index.index("infra/infra-controller.js?v=20260828a") < index.index("app.js?v=20260911a")
...
def test_iperf_controller_is_loaded_after_pure_helpers_and_owns_browser_state_machine():
    ...
    assert index.index("control/iperf-controller.js?v=20260828a") < index.index("control/delivery-panel.js?v=20260911a")
...
def test_incident_registry_owns_control_record_rendering_and_write_actions():
    ...
    assert index.index("control/incident-registry.js?v=20260828a") < index.index("app.js?v=20260911a")
...
def test_topology_browser_controller_is_extracted_without_owning_refresh_or_data_fetch():
    ...
    assert "charts/topology-panel.js?v=20260911a" in index
    assert index.index("topology.js?v=20260809a") < index.index("charts/topology-panel.js?v=20260911a")
    assert index.index("charts/topology-panel.js?v=20260911a") < index.index("app.js?v=20260911a")
...
def test_bigscreen_ping_trend_uses_job_aware_rtt_presentation():
    ...
    assert "players.js?v=20260802a" in index
    assert "api.js?v=20260911a" in index
    assert "app.js?v=20260911a" in index
    ...
    assert index.index("charts/ping-chart.js?v=20260826a") < index.index("app.js?v=20260911a")
    assert index.index("metrics/ping-transform.js?v=20260826b") < index.index("app.js?v=20260911a")
