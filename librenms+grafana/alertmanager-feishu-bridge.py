...
from feishu_bridge.device_model import (
    ...
    best_device_model as _device_model_best,
...
def _clean_device_model(value):
    return _device_model_clean(value)


def _inventory_device_model(inventory):
    return _device_model_inventory(inventory)


def _best_device_model(device):
    return _device_model_best(device)
...
_RESOURCE_WATCHER = ResourceWatcher(
    enabled=DEVICE_RESOURCE_ALERT_ENABLED,
    poll_interval=DEVICE_RESOURCE_ALERT_POLL_INTERVAL,
    cpu_alert_percent=DEVICE_CPU_ALERT_PERCENT,
    cpu_alert_for_seconds=DEVICE_CPU_ALERT_FOR_SECONDS,
    cpu_recover_percent=DEVICE_CPU_RECOVER_PERCENT,
    memory_alert_percent=DEVICE_MEMORY_ALERT_PERCENT,
    memory_alert_for_seconds=DEVICE_MEMORY_ALERT_FOR_SECONDS,
    memory_recover_percent=DEVICE_MEMORY_RECOVER_PERCENT,
    recover_seconds=DEVICE_RESOURCE_RECOVER_SECONDS,
    state_file=DEVICE_RESOURCE_STATE_FILE,
    state_lock=DEVICE_RESOURCE_STATE_LOCK,
    prometheus_query=prometheus_query,
    load_state=_load_json_dict,
    save_state=_save_json_dict,
    build_card=build_device_resource_card,
    send=send_feishu,
...
_INTERCONNECT_WATCHER = InterconnectWatcher(
    enabled=INTERCONNECT_ALERT_ENABLED,
    alert_for_seconds=INTERCONNECT_ALERT_FOR_SECONDS,
    poll_interval=INTERCONNECT_ALERT_POLL_INTERVAL,
    jobs=INTERCONNECT_ALERT_JOBS,
    port_filter=INTERCONNECT_PORT_FILTER,
    state_file=INTERCONNECT_STATE_FILE,
    prometheus_query=prometheus_query,
    fetch_name_cache=fetch_librenms_name_cache,
    load_topology_edges=load_topology_edges,
    load_json_dict=_load_json_dict,
    save_json_dict=_save_json_dict,
    as_float=_as_float,
    normalize_label=_norm_label,
    audit_port_key=_audit_port_key,
    build_card=build_interconnect_card,
    send=send_feishu,
    find_merge_candidate=find_errdisable_merge_candidate,
    complete_merge=complete_interconnect_merge,
    mark_watcher_health=mark_watcher_health,
    log=log,
)
...
def interconnect_watcher():
    return _INTERCONNECT_WATCHER.run()
