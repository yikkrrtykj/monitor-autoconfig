...
3. 把应用机器人加入目标告警群。
4. 控制台可填写共享群名称；若同名群或群列表权限受限，直接填写 `oc_` 开头的 Chat ID，跳过群列表解析。共享群中的每台 VM 必须配置唯一 `EVENT_NAME`，命令也必须带该赛事名称；`EVENT_NAME` 为空的实例会静默丢弃所有群命令（含帮助及仅 @ 机器人），不会回复，仅明确的飞书私聊（p2p）保留无前缀兼容。
